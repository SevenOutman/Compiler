...
Tree.prototype.layout = function () {
    ...
    this.nodesData.forEach(function (d) {
        d.depth -= Math.max(0, treedepth - 6);
    });

    treedepth = Math.min(6, treedepth);
...
Tree.prototype.generatePaths = function () {
    ...
    for (i = 0, nodesLength = nodesData.length; i < nodesLength; i++) {
        ...
        var p = null;
        if (parent) {
            p = canvas.path().attr({stroke: "#939598", "stroke-width": d.lit ? 3 : 0.5});
            path.push(p);
        }
        var circle = canvas.circle(startX, startY, radius)
            .attr({
                fill: d.lit ? color(d.depth / treedepth) : "#555",
                stroke: "#ffffff",
                "stroke-width": 1,
                "fill-opacity": 0.4,
                "data": 12
    ...
    nodes.forEach(function (d, i) {
        ...
        d.mouseover(function () {
                ...
                for (j = 0, textpathLength = textpath.length; j < textpathLength; j++) {
                    ...
                    var parent = nodesData[nodes[j].attr("orig")].parent;
                    //if (parent === thisparent && j !== i) {
                    if (parent === thisparent && j !== orig) {
